...
checkCoverage_modify u i =
  checkCoverage
    . cover
      30
      (UTxOIndex.member u i)
      "UTxO is a member of the index"
    . cover
      30
      (not $ UTxOIndex.member u i)
...
prop_insert_assets u b i =
  checkCoverage_modify u i
    $ UTxOIndex.assets (UTxOIndex.insert u b i)
      `Set.intersection` insertedAssets
      === insertedAssets
  where
...
prop_insert_balance u b i =
  ...
  where
    expected =
      b `TokenBundle.add` case UTxOIndex.lookup u i of
        Nothing ->
          UTxOIndex.balance i
        Just b' ->
          UTxOIndex.balance i `TokenBundle.difference` b'
...
prop_SelectionFilter_coverage selectionFilter =
  checkCoverage
    $ property
    $ cover
      20
      (category == SelectSingleton ())
      "SelectSingleton"
    $ cover
      20
      (category == SelectPairWith ())
      "SelectPairWith"
    $ cover
      20
      (category == SelectAnyWith ())
      "SelectAnyWith"
    $ cover
      20
      (category == SelectAny)
...
prop_selectRandom
  ...
  -> Property
prop_selectRandom index selectionFilter =
  monadicIO
    ...
    forAll (genAsset `suchThat` (/= a1)) $ \a2 ->
      checkCoverage $ monadicIO $ do
        ...
        monitor
          $ cover
            4
            (haveMatchForAsset1 && not haveMatchForAsset2)
        ...
        monitor
          $ cover
            4
            (haveMatchForAsset1 && haveMatchForAsset2)
...
prop_selectRandomSetMember_coversRangeUniformly i j =
  withMaxSuccess 100_000 $ checkCoverageWith confidence $ monadicIO $ do
    ...
    monitor
      $ cover
        50
        (inLowerHalf selected)
        "selected element in lower half of range"
    monitor
      $ cover
        50
        (inUpperHalf selected)
