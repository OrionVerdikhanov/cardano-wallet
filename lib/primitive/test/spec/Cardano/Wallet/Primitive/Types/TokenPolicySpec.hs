...
goldenTestCIP14 rawPolicyId rawAssetName rawFingerprint =
  it ("golden test CIP-0014 - " <> T.unpack rawFingerprint) $ do
    let
      policyId = UnsafeTokenPolicyId $ Hash $ unsafeFromHex rawPolicyId
    let
      assetName = UnsafeTokenName $ unsafeFromHex rawAssetName
    let
      fingerprint = UnsafeTokenFingerprint rawFingerprint
