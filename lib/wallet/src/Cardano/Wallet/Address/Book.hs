...
instance
  ...
  addressIso = withIso addressIso $ \from to ->
    let
      from2 st = let (a, b) = from $ Seq.innerState st in (PS a, DS b)
    ...
    where
      from (Seq.SeqState int ext a b c d e) =
        let
          ...
          ext0 = clear ext
        in
          ( SeqPrologue $ Seq.SeqState int0 ext0 a b c d e
...
-------------------------------------------------------------------------------}
-- piggy-back on SeqState existing instance, to simulate the same behavior.
instance AddressBookIso (Rnd.RndAnyState n p) where
  ...
  addressIso = withIso addressIso $ \from to ->
    let
      from2 st = let (a, b) = from $ Rnd.innerState st in (PR a, DR b)
