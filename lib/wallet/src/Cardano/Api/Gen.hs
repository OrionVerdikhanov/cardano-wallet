...
genSigningKey roletoken = do
  ...
  let
    sk = deterministicSigningKey roletoken seed
  return sk
  where
    seedSize :: Word
...
genScriptData =
  ...
  where
    ...
    recursive n = do
      k <- choose (0, n)
      let
        smallerGen = genTerm (n `div` (max k 1))
      oneof
        [ ScriptDataList
...
genExecutionUnits = do
  ...
  Large mem <- arbitrary
  let
    fromWord64 = fromIntegral @Word64
...
genCostModel = do
  let
    costModelParams = Alonzo.getCostModelParams Plutus.testingCostModelV1
  eCostModel <-
    Alonzo.mkCostModel
      <$> genPlutusLanguage
...
genTxBodyContent era = do
  ...
  let
    witnesses = collectTxBodyScriptWitnesses txBody
