...
instance IsServerError ErrWithRootKey where
  toServerError = \case
    ...
    ErrWithRootKeyWrongPassphrase wid ErrWrongPassphrase ->
      apiError err403 WrongEncryptionPassphrase
        $ mconcat
    ...
    ErrWithRootKeyWrongPassphrase wid (ErrPassphraseSchemeUnsupported s) ->
      apiError err501 WrongEncryptionPassphrase
        $ mconcat
...
instance IsServerError ErrMkTransaction where
  toServerError = \case
    ...
    ErrMkTransactionTokenQuantityExceedsLimit e ->
      apiError err403 OutputTokenQuantityExceedsLimit
        $ mconcat
...
instance IsServerError ErrBalanceTx where
  toServerError = \case
    ...
    ErrBalanceTxConflictingNetworks ->
      apiError err403 BalanceTxConflictingNetworks
        $ T.unwords
...
instance IsServerError ErrSubmitTransaction where
  toServerError = \case
    ...
    ErrSubmitTransactionPartiallySignedOrNoSignedTx expectedWitsNo foundWitsNo ->
      apiError err403 MissingWitnessesInTransaction
        $ mconcat
...
instance IsServerError ErrStartTimeLaterThanEndTime where
  toServerError err =
    apiError err400 StartTimeLaterThanEndTime
      $ mconcat
...
instance
  ...
  where
  toServerError e =
    apiError err403 OutputTokenBundleSizeExceedsLimit
      $ mconcat
...
instance IsServerError ErrSelectAssets where
  toServerError = \case
    ...
    ErrSelectAssetsAlreadyWithdrawing tx ->
      apiError err403 AlreadyWithdrawing
        $ mconcat
...
instance IsServerError ErrAssignRedeemers where
  toServerError = \case
    ...
    ErrAssignRedeemersTranslationError e ->
      apiError err400 TranslationError
        $ T.unwords
  ...
  toServerError (req, err@(ServerError code _ body _))
    | not (isJSON body) = case code of
        ...
        415 ->
          ...
          in
            apiError err UnsupportedMediaType
              $ mconcat
