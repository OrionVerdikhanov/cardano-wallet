...
toApiNetworkParameters (NetworkParameters gp sp pp) toEpochInfo = do
  ...
  let
    apiEras = ApiEraInfo {byron, shelley, allegra, mary, alonzo, babbage}
...
data ApiCoinSelectionWithdrawal (n :: NetworkDiscriminant) = ApiCoinSelectionWithdrawal
...
data ApiWalletMigrationPostData (n :: NetworkDiscriminant) (s :: Symbol) = ApiWalletMigrationPostData
    ...
    case detectEncoding (T.unpack txt) of
      Just EBech32 {} -> do
        (hrp, dp) <- case Bech32.decodeLenient txt of
          ...
          Right res -> pure res
        let
          checkPayload bytes = case xpubFromBytes bytes of
            Nothing ->
              Left
                $ TextDecodingError
        ...
        let
          proceedWhenHrpCorrect = case Bech32.dataPartToBytes dp of
            Nothing ->
...
newtype ApiMnemonicT (sizes :: [Nat]) = ApiMnemonicT {getApiMnemonicT :: SomeMnemonic}
  deriving (Generic, Eq)
...
instance ToJSON ApiCredential where
  ...
  toJSON (CredentialScriptHash (ScriptHash script)) = do
    let
      hrp = [Bech32.humanReadablePart|script|]
...
instance ToJSON ApiAddressData where
  ...
  toJSON (ApiAddressData (AddrBase payment' stake') validation') =
    object
      $ ["payment" .= payment', "stake" .= stake'] ++ addOptionally validation'
...
instance FromJSON ApiValidityBound where
  parseJSON obj = processNull <|> processObject obj
    where
      processNull =
        if obj == Aeson.Null
          then pure ApiValidityBoundUnspecified
          else fail "invalid string of ApiValidityBound"
...
instance FromJSON ApiIncompleteSharedWallet where
  parseJSON val = case val of
    Aeson.Object obj -> do
      let
        obj' = Aeson.delete (Aeson.fromText "state") obj
...
instance FromText AnyAddress where
  fromText txt = case detectEncoding (T.unpack txt) of
    Just EBech32 {} -> do
      ...
      let
        err1 = TextDecodingError "AnyAddress has invalid Bech32 datapart."
      let
        proceedWhenHrpCorrect ctr net = do
