  ...
  where
    ...
    exec (ByronWalletCreateFromMnemonicArgs wPort wName wStyle) = case wStyle of
      Random -> do
        wSeed <- do
          let
            prompt = "Please enter " ++ fmtAllowedWords wStyle ++ " : "
          let
            parser = mkSomeMnemonic @(AllowedMnemonics 'Random) . T.words
      ...
      Icarus -> do
        wSeed <- do
          let
            prompt = "Please enter " ++ fmtAllowedWords wStyle ++ " : "
          let
            parser = mkSomeMnemonic @(AllowedMnemonics 'Icarus) . T.words
          fst <$> getLine @SomeMnemonic (T.pack prompt) (left show . parser)
        wPwd <- getPassphraseWithConfirm "Please enter a passphrase: "
        runClient wPort Aeson.encodePretty
          $ postWallet mkClient
      ...
      Trezor -> do
        wSeed <- do
          let
            prompt = "Please enter " ++ fmtAllowedWords wStyle ++ " : "
          let
            parser = mkSomeMnemonic @(AllowedMnemonics 'Trezor) . T.words
          fst <$> getLine @SomeMnemonic (T.pack prompt) (left show . parser)
        wPwd <- getPassphraseWithConfirm "Please enter a passphrase: "
        runClient wPort Aeson.encodePretty
          $ postWallet mkClient
      ...
      Ledger -> do
        wSeed <- do
          let
            prompt = "Please enter " ++ fmtAllowedWords wStyle ++ " : "
          let
            parser = mkSomeMnemonic @(AllowedMnemonics 'Ledger) . T.words
          fst <$> getLine @SomeMnemonic (T.pack prompt) (left show . parser)
        wPwd <- getPassphraseWithConfirm "Please enter a passphrase: "
        runClient wPort Aeson.encodePretty
          $ postWallet mkClient
...
getMnemonics = do
  wSeed <- do
    let
      prompt = "Please enter the 15–24 word recovery phrase: "
    let
      parser = mkSomeMnemonic @'[15, 18, 21, 24] . T.words
              ...
              $ let
                  ...
                  mnemonicA (wMnemonic, wSndFactor) =
                    WalletPutPassphraseMnemonicData
                      ...
                      (ApiT wPassphraseNew)
                in
                  bimap oldPassA mnemonicA wCredentials
          Left _ ->
...
sendRequest (Port p) cmd = do
  ...
  let
    env = mkClientEnv manager (BaseUrl Http "localhost" p "")
  runClientM cmd env
...
getPassphraseWithConfirm prompt = do
  ...
  (wPwd', _) <- do
    let
      promptRepeat = "Enter the passphrase a second time: "
    let
      parser = fromText @(Passphrase a)
...
getDataDir backendDir = do
  ...
  let
    dir = if os /= "windows" then XdgData else XdgCache
