...
specWithServer testDir (tr, tracers) = aroundAll withContext
  where
    ...
    withContext action = bracketTracer' tr "withContext" $ do
      ...
      let
        setupContext smashUrl conn np baseUrl = bracketTracer' tr "setupContext" $ do
          prometheusUrl <-
          ...
          let
            fiveMinutes = 300 * 1_000 * 1_000 -- 5 minutes in microseconds
          manager <-
            newManager
              $ defaultManagerSettings
      ...
      let
        action' = bracketTracer' tr "spec" . action
      res <-
        race
        ...
        decorate Pool.DBLayer {..} =
          ...
          where
            removeRetiredPoolsDecorated epochNo = do
              ...
              let
                event = PoolGarbageCollectionEvent epochNo certificates
              liftIO $ do
    ...
    onClusterStart action dbDecorator (RunningNode conn block0 (gp, vData) genesisPools) = do
      ...
      let
        testMetadata = $(getTestData) </> "token-metadata.json"
...
withTracers testDir action = do
  let
    getLogOutputs getMinSev name = do
      minSev <- getMinSev
  ...
  withLogging walletLogOutputs $ \(sb, (cfg, walTr)) -> do
    ...
    withLogging testLogOutputs $ \(_, (_, testTr)) -> do
      let
        trTests = appendName "integration" testTr
      let
        tracers = setupTracers (tracerSeverities (Just Debug)) walTr
