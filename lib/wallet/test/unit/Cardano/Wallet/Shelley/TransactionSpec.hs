...
prop_signTransaction_preservesScriptIntegrity (AnyCardanoEra era) rootK utxo =
  withMaxSuccess 10
    $ whenSupportedInEra Cardano.scriptDataSupportedInEra era
    $ \_supported ->
...
decodeSealedTxSpec = describe "SealedTx serialisation/deserialisation" $ do
  it "tx with withdrawal" $ do
    let
      bytes = unsafeFromHex byteString
    let
      sealedTx = sealedTxFromBytes bytes
...
feeEstimationRegressionSpec = describe "Regression tests" $ do
  it "#1740 Fee estimation at the boundaries" $ do
    let
      requiredCost = Fee (Coin.fromNatural 166_029)
    let
      estimateFee =
        except
          $ Left
  ...
  describe "Byron witnesses - mainnet" $ do
    ...
    it "1 input, 2 outputs" $ do
      ...
      let
        amtOut = 2_000_000
      let
        amtChange = amtInp - amtOut - amtFee
      let
        utxo =
          UTxO
            $ Map.fromList
    ...
    it "2 inputs, 3 outputs" $ do
      ...
      let
        amtOut = 6_000_000
      let
        amtChange = 2 * amtInp - 2 * amtOut - amtFee
      let
        utxo =
          UTxO
            $ Map.fromList
      ...
      where
        ...
        unsigned =
          either (error . show) id
            $ mkUnsignedTx
              (shelleyBasedEraFromRecentEra era)
              (Nothing, slotNo)
              (Right cs)
              md
              mempty
  ...
  dummyProtocolParameters
    ...
    , minimumUTxO =
        minimumUTxOForShelleyBasedEra Cardano.ShelleyBasedEraAlonzo
          $ def
          & ppCoinsPerUTxOWordL .~ testParameter_coinsPerUTxOWord_Alonzo
...
balanceTransactionSpec = describe "balanceTransaction" $ do
  ...
  describe "bootstrap witnesses" $ do
    ...
    let
      coinSelectionEstimatedSize :: Natural -> Natural
      coinSelectionEstimatedSize = unTxSize . sizeOf_BootstrapWitnesses

    let
      measuredWitSize :: IsCardanoEra era => Cardano.Tx era -> Natural
    ...
    let
      ...
      evaluateMinimumFeeSize (Cardano.Tx body wits) =
        ...
        where
          ...
          -- size-delta.
          pp =
            Write.withConstraints (Write.recentEra @era)
              $ Ledger.emptyPParams
              & set ppMinFeeAL (Ledger.Coin 1)

    let
      evaluateMinimumFeeDerivedWitSize (Cardano.Tx body wits) =
        evaluateMinimumFeeSize (Cardano.Tx body wits)
    ...
    it "coin-selection's size estimation == balanceTx's size estimation"
      ...
      $ \n (Write.InAnyRecentEra _era tx) -> do
        let
          balanceSize = evaluateMinimumFeeDerivedWitSize tx
        let
          csSize = coinSelectionEstimatedSize $ intCast n
        balanceSize === csSize
    ...
    it "balanceTx's size estimation >= measured serialized size"
      ...
      $ \n (Write.InAnyRecentEra _era tx) -> do
        let
          estimated = evaluateMinimumFeeDerivedWitSize tx
        let
          measured = measuredWitSize tx
        let
          overestimation
  ...
  describe "change address generation" $ do
    ...
    let
      address :: Babbage.BabbageTxOut StandardBabbage -> Address
    ...
    it "assigns change addresses as expected"
      ...
      `shouldBe` (map (view #address) paymentOuts ++ expectedChange)
  ...
  it "increases zero-ada outputs to minimum" $ do
    ...
    let
      era = Write.RecentEraBabbage
    let
      out = TxOut dummyAddr (TokenBundle.fromCoin (Coin 0))
    let
      out' = TxOut dummyAddr (TokenBundle.fromCoin (Coin 874_930))
    let
      Cardano.ShelleyTx _ tx =
        either (error . show) id
          $ balance
          $ paymentPartialTx [out]
    let
      outs = Write.outputs era $ Write.txBody era tx
  ...
  describe "when passed unresolved inputs" $ do
    it "fails with ErrBalanceTxUnresolvedTxIn" $ do
      let
        txin = TxIn (Hash $ B8.replicate 32 '3') 10

      -- 1 output, 1 input without utxo entry
      let
        partialTx :: PartialTx Cardano.BabbageEra
        partialTx =
          addExtraTxIns [txin]
            $ paymentPartialTx
              [ TxOut
      ...
      balance partialTx
        `shouldBe` Left (ErrBalanceTxUnresolvedInputs (txin :| []))

    describe "with redeemers"
      $ it "fails with ErrBalanceTxUnresolvedInputs"
      $ do
        let
          withNoUTxO :: PartialTx era -> PartialTx era
  ...
  describe "when a redeemer is missing" $ do
    it "balancing succeeds (currently)" $ do
      ...
      let
        withNoRedeemers = over #redeemers (const [])
...
distributeSurplusSpec = do
  describe "sizeOfCoin" $ do
    let
      coinToWord64Clamped = fromMaybe maxBound . Coin.toWord64Maybe
    let
      cborSizeOfCoin =
        TxSize
          . fromIntegral
    ...
    it "matches the size of the Word64 CBOR encoding"
      $ property
      $ checkCoverage
      $ forAll genEncodingBoundaryLovelace
      $ \l -> do
        let
          c = fromCardanoLovelace l
        let
          expected = cborSizeOfCoin c

        -- Use a low coverage requirement of 0.01% just to
        -- ensure we see /some/ amount of every size.
        let
          coverSize s = cover 0.01 (s == expected) (show s)
  ...
  describe "costOfIncreasingCoin" $ do
    it
      ...
      $ do
        let
          expectedCostIncrease = Coin 176
        let
          mainnet = mainnetFeePerByte
    ...
    it "produces results in the range [0, 8 * feePerByte]"
      ...
      $ \c increase -> do
        let
          res = costOfIncreasingCoin (FeePerByte 1) c increase
  ...
  describe "distributeSurplusDelta" $ do
    ...
    describe "when increasing change increases fee"
      ...
      `shouldBe` Right (TxFeeAndChange (Coin 1) [Coin 99])
    ...
    describe "when increasing fee increases fee"
      ...
      $ do
        distributeSurplusDelta
          ...
          `shouldBe` Right (TxFeeAndChange (Coin 2) [Coin 98])
      ...
      $ do
        it "will try burning the surplus as fees" $ do
          distributeSurplusDelta
            ...
            `shouldBe` Right (TxFeeAndChange (Coin 10) [Coin 0])
        ...
        it "will fail if neither the fee can be increased" $ do
          distributeSurplusDelta
            ...
            `shouldBe` Left (ErrMoreSurplusNeeded $ Coin 34)
  ...
  prop_distributeSurplus_onSuccess
    $ \policy
       ...
       (TxFeeAndChange feeModified changeModified) -> do
        let
          coinsOriginal = feeOriginal : (TxOut.coin <$> changeOriginal)
        let
          coinsModified = feeModified : (TxOut.coin <$> changeModified)
        let
          coinDeltas = zipWith Coin.difference coinsModified coinsOriginal
        let
          costIncrease =
            F.foldMap
...
prop_distributeSurplus_onSuccess_increasesValuesByDelta =
  prop_distributeSurplus_onSuccess
    $ \policy
       ...
       (TxFeeAndChange feeModified changeModified) ->
        let
          (TxFeeAndChange feeDelta changeDeltas) =
            either (error . show) id
...
instance Arbitrary (PartialTx Cardano.BabbageEra) where
  arbitrary = do
    ...
    let
      (Cardano.Tx (Cardano.TxBody content) _) = tx
    let
      inputs = Cardano.txIns content
...
shrinkTxBodyBabbage (Cardano.ShelleyTxBody e bod scripts scriptData aux val) =
  ...
  where
    ...
    shrinkValidity (ValidityInterval a b) =
      tail
...
instance Buildable BalanceTxGolden where
  build (BalanceTxGoldenFailure c err) =
    mconcat
      [ build c
      , ","
      , build (T.pack err)
      ]
  build (BalanceTxGoldenSuccess c fee minfee) =
    mconcat
      [ build c
      , ","
      , lovelaceF fee
...
balanceTransactionGoldenSpec = describe "balance goldens" $ do
  ...
  describe "balanced binaries" $ do
    let
      dir = $(getTestData) </> "balanceTx" </> "binary" </> "balanced"
    let
      walletUTxO = utxo [Coin 5_000_000]
    it "pingPong_2" $ do
      let
        ptx = pingPong_2
      let
...
prop_balanceTransactionValid
  ...
  seed =
    withMaxSuccess 1_000 $ do
      ...
      let
        originalBalance = txBalance (view #tx partialTx) combinedUTxO
      let
        originalOuts = txOutputs (view #tx partialTx)
      let
        classifications =
          classify
            (hasZeroAdaOutputs $ view #tx partialTx)
      ...
      classifications $ case res of
        Right tx -> counterexample ("\nResult: " <> show (Pretty tx)) $ do
          label "success"
            $ classify
              ...
              "already balanced"
            $ classify
              (txFee tx > Cardano.Lovelace 1_000_000)
              "fee above 1 ada"
            $ classify
              (hasCollateral tx)
        ...
        Left
          ( ErrBalanceTxSelectAssets
              ( ErrSelectAssetsSelectionError
                  (SelectionBalanceErrorOf (BalanceInsufficient err))
                )
            ) -> do
            let
              missing = view #utxoBalanceShortfall err
            let
              missingCoin = view #coin missing == Coin 0
            let
              missingTokens = view #tokens missing == mempty
    ...
    where
      ...
      prop_expectFeeExcessSmallerThan lim tx utxo = do
        let
          fee = txFee tx
        let
          minfee = minFee tx utxo
        let
          unLovelace (Cardano.Lovelace x) = x
        let
          delta = Cardano.Lovelace $ (unLovelace fee) - (unLovelace minfee)
        let
          msg =
            unwords
              [ "The fee"
      ...
      prop_minfeeIsCovered tx utxo = do
        ...
        let
          unLovelace (Cardano.Lovelace x) = x
        let
          delta = Cardano.Lovelace $ (unLovelace minfee) - (unLovelace fee)
        let
          msg =
            unwords
              [ "The minimum fee was"
      ...
      _prop_outputsSatisfyMinAdaRequirement (Cardano.ShelleyTx _ tx) = do
        let
          outputs = Write.outputs era $ Write.txBody era tx
        ...
        where
          ...
          valid out =
            counterexample msg
              $ property
              $ not
              $ Write.isBelowMinimumCoinForTxOut era ledgerPParams out
            where
              msg =
                unwords
                  [ "ada quantity is"
      ...
      minFee
        ...
        -> Cardano.UTxO era
        -> Cardano.Lovelace
      minFee tx@(Cardano.Tx body _) utxo =
        Write.toCardanoLovelace
          $ Write.evaluateMinimumFee
            (recentEra @era)
            ledgerPParams
...
prop_balanceTransactionExistingTotalCollateral
  wallet
  (ShowBuildable partialTx@PartialTx {tx})
  seed =
    withMaxSuccess 10
...
prop_bootstrapWitnesses
  p
  n
  (AnyRecentEra (era :: RecentEra era))
  net
  accIx
  addr0Ix =
    let
...
costModelsForTesting = unsafePerformIO $ do
  let
    fp = $(getTestData) </> "cardano-node-shelley" </> "alonzo-genesis.yaml"
            ...
            case updateTx tx noTxUpdate of
              ...
              Right tx' -> do
                if tx /= tx' && show tx == show tx'
                  then -- The transaction encoding has changed.
                  ...
                  do
                    ...
                    expectationFailure
                      $ "Transaction encoding has changed, making comparison impossible, see .rej file: "
                        <> rejectFilePath
                  else
...
prop_updateTx
  ...
  newFee =
    do
      let
        extra = TxUpdate extraIns extraCol extraOuts [] (UseNewTxFee newFee)
      let
        tx' =
          either (error . show) id
            $ updateTx tx extra
...
estimateSignedTxSizeSpec = describe "estimateSignedTxSize" $ do
  ...
  where
    ...
    test _name bs tx@(Cardano.Tx (body :: Cardano.TxBody era) _) = do
      let
        pparams = Write.pparamsLedger $ mockPParamsForBalancing @era
        ...
        noScripts =
          Write.withConstraints (recentEra @era)
            $ Map.null
            $ ledgerTx ^. witsTxL . scriptTxWitsL
        noBootWits =
          Write.withConstraints (recentEra @era)
            $ Set.null
            $ ledgerTx ^. witsTxL . bootAddrTxWitsL
    ...
    forAllGoldens goldens f = forM_ goldens $ \(name, bs) -> it name
      $ Write.withInAnyRecentEra (recentEraTxFromBytes bs)
      $ \tx ->
        let
          msg =
            unlines
    ...
    { tx =
        deserializeBabbageTx
          $ mconcat
            ...
            , tid
            , unsafeFromHex
                ""
            ]
    , inputs =
        Write.toCardanoUTxO
