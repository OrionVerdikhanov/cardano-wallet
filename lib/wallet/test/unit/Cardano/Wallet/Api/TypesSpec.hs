...
spec = do
  ...
  describe "SealedTx JSON decoding" $ do
    ...
    it "can decode from base-16 encoded string"
      ...
      $ \bs ->
        let
          result = parseJSONSealedTx $ Aeson.encode $ ApiBytesT @'Base16 bs
        in
          result == Right bs
    ...
    it "can decode from base-64 encoded string"
      ...
      $ \bs ->
        let
          result = parseJSONSealedTx $ Aeson.encode $ ApiBytesT @'Base64 bs
        in
          result == Right bs
            & counterexample ("Parse result: " <> show result)
  ...
  describe "verify JSON parsing failures too" $ do
    ...
    it "ApiT (Passphrase \"user\") (too short)" $ do
      let
        minLength = passphraseMinLength (Proxy :: Proxy "user")
      let
        msg =
    ...
    it "ApiT (Passphrase \"user\") (too long)" $ do
      let
        maxLength = passphraseMaxLength (Proxy :: Proxy "user")
      let
        msg =
    ...
    it "ApiT (Passphrase \"lenient\") (too long)" $ do
      let
        maxLength = passphraseMaxLength (Proxy :: Proxy "lenient")
      let
        msg =
          "Error in $: passphrase is too long: \
          \expected at most "
    ...
    it "ApiT DerivationIndex (too small)" $ do
      ...
      let
        value = show $ pred $ toInteger $ getIndex @'Soft minBound
    ...
    it "ApiT DerivationIndex (too large)" $ do
      ...
      let
        value = show $ succ $ toInteger $ getIndex @'Soft maxBound
      Aeson.parseEither parseJSON [aesonQQ|#{value}|]
        `shouldBe` Left @String @(ApiT DerivationIndex) message
    ...
    describe "StakePoolMetadata" $ do
      ...
      let
        testInvalidTicker :: Text -> SpecWith ()
  ...
  describe "Api Errors" $ do
    it "Every ApiErrorInfo constructor has a corresponding schema type"
      $ let
          res = fromJSON @SchemaApiErrorInfo specification
          errStr = case res of
            Error s -> s
            _ -> ""
        in
          counterexample errStr $ res == Success SchemaApiErrorInfo
...
instance Arbitrary ApiScriptTemplateEntry where
  arbitrary = do
    ...
    let
      scriptCosigners = retrieveAllCosigners script
...
instance Arbitrary ApiSharedWalletPostData where
  arbitrary = do
    let
      fromMnemonics = arbitrary :: Gen ApiSharedWalletPostDataFromMnemonics
    let
      fromAccXPub = arbitrary :: Gen ApiSharedWalletPostDataFromAccountPubX
    oneof
...
instance Arbitrary WalletOrAccountPostData where
  arbitrary = do
    let
      walletPostDataGen = arbitrary :: Gen WalletPostData
    let
      accountPostDataGen = arbitrary :: Gen AccountPostData
    oneof
...
instance Arbitrary ApiTokenAmountFingerprint where
  arbitrary = do
    ...
    policyid <- arbitrary
    let
      fingerprint = ApiT $ mkTokenFingerprint policyid name
...
instance Arbitrary ApiTokens where
  arbitrary = do
    ...
    let
      keyhash = KeyHash Policy $ getHash $ unTokenPolicyId policyid
    script <-
      elements
...
selectFromPreparedBinaries =
  ...
  where
    toByteString txt =
      let
        (Right bs) = fromHex $ T.encodeUtf8 txt
      in
        bs
  ...
  arbitrary = do
    ...
    where
      ...
      genEntry = do
        ...
        let
          ada = Coin.toQuantity $ max adaValue1 adaValue2
        let
          adaMinimum = Coin.toQuantity $ min adaValue1 adaValue2
...
instance Arbitrary ApiUtxoStatistics where
  arbitrary = do
    utxos <- arbitrary
    let
      (UTxOStatistics histoBars stakes bType) =
...
specification =
  ...
  where
    bytes =
      $( let
          swaggerYaml = "./specifications/api/swagger.yaml"
         in
          liftIO (lookupEnv "SWAGGER_YAML")
...
declareSchemaForDefinition ref = do
  let
    json = foldl' unsafeLookupKey specification ["components", "schemas", ref]
...
instance
  ...
  extractEndpoints _ prefixes = do
    let
      prefixes' = symbolVal (Proxy @path) : prefixes
...
instance {-# OVERLAPPABLE #-} GetPath a => ExtractEndpoints a where
  extractEndpoints proxy prefixes = do
    let
      (verb, subPath) = first show (getPath proxy)
