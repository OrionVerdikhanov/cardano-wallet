...
spec = do
  ...
  describe "AddressPoolGap - Text Roundtrip" $ do
    ...
    let
      err = "An address pool gap must be a natural number between 10 and 100000."
  ...
  where
    prop_addrsNotInInternalPool addrs =
      map
        ( \(x, s) ->
            let
              notInPool = isNothing $ lookupAddress x intPool
              isUsed = s == Used
            in
              (ShowFmt x, notInPool || isUsed)
        )
        addrs
...
changeAddresses as s =
  let
    (a, s') = genChange (\k _ -> paymentAddress SMainnet k) s
  in
    if a `elem` as then (as, s) else changeAddresses (a : as) s'
...
instance
  ...
  arbitrary = do
    ...
    let
      SeqAddressPool pool = newSeqAddressPool @'Mainnet @c ourAccount gap
...
instance Arbitrary UnexpectedPrefix where
  arbitrary = do
    let
      baseAddr = 0b00000001 -- keyhash; keyhash, mainnet
