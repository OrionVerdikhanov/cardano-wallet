...
spec = do
  describe "Goldens" $ do
    let
      acc = toRewardAccount @StakeKey' . toEnum
    let
      regAndDeleg i =
        applyTx
          ( Tx
...
prop_keysConsecutive f cmds = do
  let
    env = applyCmds env0 cmds
  let
    keys = map fromEnum $ f $ wallet env
...
prop_usableKeysAdversaries cmds = do
  counterexample "\nstate /= state without adversarial cmds" $ do
    let
      usableKeys' = usableKeys . wallet . applyCmds env0
    usableKeys' cmds
...
prop_pointerAdversaries (NonNegative n) keys = do
  let
    env = applyCmds env0 [CmdSetPortfolioOf n]
  let
    env' = applyCmds env (map CmdMimicPointerOutput keys)
...
prop_canAlwaysGoTo0 cmds = do
  let
    env = applyCmds env0 (cmds ++ [CmdSetPortfolioOf 0])
...
prop_rollbacks (NonNegative n) cmds = do
  forAllSubchains (applyCmds env0 cmds) $ \env' -> do
    let
      env = applyCmds env' [CmdSetPortfolioOf n]
...
instance PaymentAddress StakeKey' 'CredFromKeyK where
  ...
  paymentAddress _ k =
    let
      FromKeyHash bs = toRewardAccount k
    in
      Address $ "addr" <> bs
