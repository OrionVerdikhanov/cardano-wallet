      ...
      let
        ...
        (txins, txouts) =
          unzip
            ...
            , let
                txin = (W.TxIn txId txOutPos, Just txout)
            ]
      let
        guinea' = set #resolvedInputs txins guinea
      pure (guineaId, mkTxSet (guinea' : transactions), txouts)

  forAll transactionsGen $ \(txid, TxSet pile, txouts) ->
    let
      guinea = pile Map.! txid
      deco = decorateTxInsForRelation' (TxSet pile) guinea
    in
      [lookupTxOut (mkTxOutKey txin) deco | txin <- ins guinea]
        === map Just txouts
...
prop_DecorateLinksTxCollateralsToTxOuts = do
  let
    transactionsGen = do
      ...
      let
        guinea' = set #resolvedCollateralInputs txins guinea
      pure (guineaId, mkTxSet (guinea' : transactions), txouts)
  ...
  forAll transactionsGen $ \(txid, TxSet pile, txouts) ->
    let
      ...
      deco = decorateTxInsForRelation' (TxSet pile) guinea
    in
      [ lookupTxOut (mkTxOutKeyCollateral txcol) deco
