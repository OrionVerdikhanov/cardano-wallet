...
instance MockPrivKey (ShelleyKey 'RootK) where
    fromMockPrivKey s = (k, unMockPrivKeyHash s)
      where
        (k, _)
            = unsafeDeserializeXPrv (keyFlavor @ShelleyKey) (zeroes, mempty)

instance MockPrivKey (SharedKey 'RootK) where
    fromMockPrivKey s = (k, unMockPrivKeyHash s)
      where
        (k, _)
            = unsafeDeserializeXPrv (keyFlavor @SharedKey) (zeroes, mempty)
...
instance MockPrivKey (ByronKey 'RootK) where
    fromMockPrivKey s = (k, unMockPrivKeyHash s)
      where
        (k, _)
            = unsafeDeserializeXPrv (keyFlavor @ByronKey)
                (zeroes <> ":", mempty)
