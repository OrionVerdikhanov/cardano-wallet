...
properties withFreshDB = describe "DB.Properties" $ do
  let
    testOnLayer = monadicIO . withFreshWallet testWid withFreshDB
...
prop_createWalletTwice test (wid, InitialCheckpoint cp0, meta) = monadicIO
  ...
  $ \DBFresh {..} -> do
    liftIO $ do
      let
        err = ErrWalletAlreadyInitialized
...
prop_rollbackTxHistory test (InitialCheckpoint cp0) (GenTxHistory txs0) = do
  ...
  where
    ...
    forgotten slot =
      let
        isAfter meta = meta ^. #slotNo > slot
      in
        filterTxs isAfter txs0
