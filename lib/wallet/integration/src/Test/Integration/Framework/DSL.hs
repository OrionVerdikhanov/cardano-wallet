...
expectListField i getter predicate (c, res) = liftIO $ case res of
  ...
  Right xs
    ...
    | otherwise ->
        expectationFailure
          $ "expectListField: trying to access the #"
            <> show i
...
expectCliListField i getter predicate xs
  ...
  | otherwise =
      liftIO . expectationFailure
        $ "expectCliListField: trying to access the #"
          <> show i
          <> " element from a list but there's none! "
...
derivationPathValidationErrors
  ...
  -> NonEmpty (ApiT DerivationIndex)
  -> [String]
derivationPathValidationErrors purpose path =
  catMaybes
    [ let
        l = length path
      in
    ...
    , if purposeIdx /= Just (ApiT (DerivationIndex (getIndex purpose)))
        then
          Just
            ( "Derivation path must start with a purpose "
                <> show purpose
                <> " but it has "
                <> show purposeIdx
            )
    ...
    , let
        minAccount = getIndex @'Hardened minBound
      in
...
genXPubs num =
  ...
  where
    genXPub = do
      ...
      let
        accXPubTxt = accPubKeyFromMnemonics m15 (Just m12) 10 mempty
      let
        (Just accXPub) = xpubFromText accXPubTxt
  ...
  where
    genXPub = do
      ...
      let
        (Right m12) = mkSomeMnemonic @'[12] m12txt
      let
        accXPubTxt = sharedAccPubKeyFromMnemonics m15 (Just m12) 10 mempty
      let
        (Just accXPub) = xpubFromText accXPubTxt
      return (accXPub, accXPubTxt)
...
waitForTxImmutability _ctx = liftIO $ do
  ...
  let
    stabilityDelay = 6 * oneSecond
  let
    txInsertionDelay = 4 * oneSecond
...
eventuallyUsingDelay
  ...
  -- ^ Timeout in seconds
  -> String
  -- ^ Brief description of the IO action
  -> IO a
  -> m a
...
emptyRandomWalletWithPasswd ctx rawPwd = do
  let
    pwd =
      preparePassphrase W.EncryptWithScrypt
        $ Passphrase
        $ BA.convert
        $ T.encodeUtf8 rawPwd
...
fixtureMultiAssetRandomWallet ctx = do
  ...
  rL <-
    request @[ApiAddressWithPath n]
      ctx
      (Link.listAddresses @'Byron wB)
      Default
      Empty
  let
    addrs = getFromResponse id rL
  let
    destination = (addrs !! 1) ^. #id
  ...
  rtx <-
    request @(ApiTransaction n)
      ctx
      (Link.createTransactionOld @'Shelley wMA)
      Default
      payload
...
fundSharedWallet ctx amt sharedWals = do
  let
    wal = case NE.head sharedWals of
  ...
  let
    sharedAddrs = getFromResponse Prelude.id rAddr
  let
    destination = (sharedAddrs !! 1) ^. #id
  ...
  expectResponseCode HTTP.status202 rTx
  let
    fee = unsafeResponse rTx ^. #fee . #getQuantity
...
postSharedWallet ctx headers payload = snd <$> allocate create (free . snd)
  where
    ...
    free (Right (ApiSharedWallet (Left w))) =
      void
        $ request @Aeson.Value
          ctx
          ...
          Default
          Empty
    free (Right (ApiSharedWallet (Right w))) =
      void
        $ request @Aeson.Value
          ctx
          (Link.deleteWallet @'Shared w)
...
getSharedWalletKey ctx wal role ix isHashed =
  ...
  where
    r
      ...
      -> m (HTTP.Status, Either RequestException ApiVerificationKeyShared)
    r w =
      request @ApiVerificationKeyShared
        ctx
        (Link.getWalletKey @'Shared w role ix isHashed)
        Default
        Empty
  ...
  where
    ...
    r w =
      request @ApiAccountKeyShared
        ctx
        (Link.postAccountKey @'Shared w ix)
        headers
        payload
...
getAccountKeyShared ctx wal isHashed =
  ...
  where
    ...
    r w =
      request @ApiAccountKeyShared
        ctx
        (Link.getAccountKey @'Shared w isHashed)
        Default
        Empty
...
patchSharedWallet ctx wal cred payload =
  ...
  where
    ...
    r w =
      let
        endpoint = "v2/shared-wallets" </> w ^. walletId </> patchEndpointEnding cred
      in
...
fixtureRandomWalletWith ctx coins0 = do
  ...
  let
    addrs = randomAddresses @n mws
  liftIO $ mapM_ (moveByronCoins @n ctx src (dest, addrs)) (groupsOf 10 coins0)
  void
    $ request @()
      ctx
      (Link.deleteWallet @'Byron src)
      Default
      Empty
  r <-
    request @ApiByronWallet
      ctx
      (Link.getWallet @'Byron dest)
      Default
      Empty
  expectResponseCode HTTP.status200 r
      ...
      balance <-
        getFromResponse (#balance . #available . #getQuantity)
          <$> request @ApiWallet
            ...
            Empty
      addrs <-
        fmap (view #id) . getFromResponse id
          <$> request @[ApiAddressWithPath n]
            ctx
            (Link.listAddresses @'Shelley dest)
            Default
            Empty
      let
        payments = for (zip coins addrs) $ \(amt, addr) ->
          [aesonQQ|{
                "address": #{addr},
                "amount": {
                    "quantity": #{amt},
...
moveByronCoins ctx src (dest, addrs) coins = do
  ...
  let
    payments = for (zip coins addrs) $ \(amt, addr) ->
      let
        addrStr = encodeAddress (sNetworkId @n) addr
      in
        [aesonQQ|{
                "address": #{addrStr},
                "amount": {
  ...
  request @(ApiTransaction n)
    ctx
    (Link.createTransactionOld @'Byron src)
    Default
    payload
    >>= expectResponseCode HTTP.status202
  eventually "balance available = balance total" $ do
...
getFromResponseList i getter (_, res) = case res of
  ...
  Right xs
    ...
    | otherwise ->
        error
          $ "getFromResponseList: trying to access the #"
            <> show i
            <> " element from a list but there's none! "
...
joinStakePool ctx p (w, pass) = do
  let
    payload = Json [aesonQQ| { "passphrase": #{pass} } |]
  request @(ApiTransaction n)
    ctx
    (Link.joinStakePool (Identity p) w)
    Default
    payload
...
quitStakePoolUnsigned ctx w = liftIO $ do
  let
    payload = Json [aesonQQ|{ "delegation_action": { "action": "quit" } }|]
  request @(ApiCoinSelection n)
    ctx
    (Link.selectCoins @style w)
    Default
...
signSharedTx ctx w sealedTx expectations = do
  ...
  let
    signEndpoint = Link.signTransaction @'Shared w
  r <- request @ApiSerialisedTransaction ctx signEndpoint Default toSign
  verify r expectations
  pure $ getFromResponse Prelude.id r
...
submitTx ctx tx expectations = do
  let
    bytes = serialisedTx $ getApiT (tx ^. #serialisedTxSealed)
  let
    submitEndpoint = Link.postExternalTransaction
  let
    headers =
      Headers
...
submitTxWithWid ctx w tx = do
  let
    submitEndpoint = Link.submitTransaction @'Shelley w
  let
    payload = Json $ Aeson.toJSON tx
...
submitSharedTxWithWid ctx w tx = do
  let
    submitEndpoint = Link.submitTransaction @'Shared w
  let
    payload = Json $ Aeson.toJSON tx
  request @ApiTxId ctx submitEndpoint Default payload
...
deleteAllWallets ctx = do
  ...
  forM_ (wallets (snd resp)) $ \wal -> do
    let
      endpoint = "v2/wallets" </> wal ^. walletId
  ...
  forM_ (wallets (snd respByron)) $ \wal -> do
    let
      endpoint = "v2/byron-wallets" </> wal ^. walletId
    d <- request @Value ctx ("DELETE", endpoint) None Empty
    expectResponseCode HTTP.status204 d
  where
...
listFilteredSharedWallets include ctx = do
  (s, mwallets) <-
    request @[ApiSharedWallet]
      ctx
      (Link.listWallets @'Shared)
      Default
      Empty
  return
    ( s
    , filter (\w -> (getWalletIdFromSharedWallet w ^. walletId) `Set.member` include)
        <$> mwallets
    )
...
waitForServer
  :: forall ctx m
   . (HasType (Port "wallet") ctx, MonadIO m)
  => ctx
  -> m ()
waitForServer ctx =
  liftIO
...
createWalletViaCLI
  :: forall s m
   . (HasType (Port "wallet") s, MonadIO m)
  => s
  -> [String]
  -> String
  -> String
...
createWalletViaCLI ctx args mnemonics secondFactor passphrase =
  ...
  where
    create = do
      ...
      let
        process = proc' commandName fullArgs
      liftIO
...
getWalletUtxoStatisticsViaCLI
  ...
  -> String
  -> m r
getWalletUtxoStatisticsViaCLI ctx walId =
  cardanoWalletCLI
...
getWalletUtxoSnapshotViaCLI
  ...
  -> String
  -> m r
getWalletUtxoSnapshotViaCLI ctx walId =
  cardanoWalletCLI
    [ "wallet"
    , "utxo-snapshot"
    , "--port"
  ...
  let
    execArgs =
      ...
      , show (ctx ^. typed @(Port "wallet"))
        ...
        ++ args
  let
    process = proc' commandName execArgs
  liftIO
    $ withCreateProcess process
    $ \(Just stdin) (Just stdout) (Just stderr) h -> do
      hPutStr stdin (pass <> "\n")
...
importAddressViaCLI
  :: forall r s m
   . (CmdResult r, HasType (Port "wallet") s, MonadIO m)
  => s
  -> [String]
  -- ^ Args
  -> m r
  -- ^ (ExitCode, StdOut, StdErr)
importAddressViaCLI ctx args =
  cardanoWalletCLI
...
listTransactionsViaCLI
  ...
  -> [String]
  -> m r
listTransactionsViaCLI ctx metadataSchema args =
  cardanoWalletCLI
    $ join
      [ ["transaction", "list"]
      , ["--port", show (ctx ^. typed @(Port "wallet"))]
...
postExternalTransactionViaCLI
  ...
  -> [String]
  -> m r
postExternalTransactionViaCLI ctx args =
  cardanoWalletCLI
    $ join
      [ ["transaction", "submit"]
      , ["--port", show (ctx ^. typed @(Port "wallet"))]
...
getTransactionViaCLI
  ...
  -> TxMetadataSchema
  -> m r
getTransactionViaCLI ctx wid tid metadataSchema =
  cardanoWalletCLI
    $ join
      [ ["transaction", "get"]
      , ["--port", show (ctx ^. typed @(Port "wallet")), wid, tid]
...
getSlotParams ctx = do
  ...
  let
    endpoint = ("GET", "v2/network/parameters")
  r2 <- liftIO $ request @ApiNetworkParameters ctx endpoint Default Empty
  let
    (Quantity slotL) = getFromResponse #slotLength r2
  let
    (Quantity epochL) = getFromResponse #epochLength r2
  let
    (Quantity coeff) = getFromResponse #activeSlotCoefficient r2
  let
    (Quantity k) = getFromResponse #securityParameter r2
  let
    sp =
      SlottingParameters
        (SlotLength slotL)
...
getTTLSlots ctx dt = liftIO $ do
  ...
  let
    slotLen = 0.2 -- fixme: this is the value from byron genesis
