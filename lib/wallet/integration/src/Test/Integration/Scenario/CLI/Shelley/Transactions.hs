...
spec = describe "SHELLEY_CLI_TRANSACTIONS" $ do
  it "TRANS_CREATE_01 - Can create transaction via CLI" $ \ctx -> runResourceT $ do
    ...
    let
      amt = fromIntegral . minUTxOValue . _mainEra $ ctx
  ...
  it "TRANS_CREATE_02 - Multiple Output Tx to single wallet via CLI" $ \ctx -> runResourceT $ do
    ...
    let
      addr1 = encodeAddress (sNetworkId @n) (apiAddress $ addr !! 1 ^. #id)
    let
      addr2 = encodeAddress (sNetworkId @n) (apiAddress $ addr !! 2 ^. #id)
    let
      amt = fromIntegral . minUTxOValue . _mainEra $ ctx
    let
      args =
  ...
  it
    ...
    $ \ctx ->
      runResourceT $ do
        ...
        let
          srcWalletId = T.unpack (srcWallet ^. walletId)
        dstWallet <- emptyWallet ctx
        let
          amount = 10_000_000
        let
          metadata = Just "{ \"1\": { \"string\": \"hello\" } }"
        let
          txMetadata = TxMetadata $ Map.singleton 1 (TxMetaText "hello")
        let
          successfulJsonCliResponse proxy f = do
  ...
  describe "TRANS_LIST_02 - Start time shouldn't be later than end time"
    ...
    $ \mOrder -> do
      let
        startTime = max validTime1 validTime2
      let
        endTime = min validTime1 validTime2
      let
        title =
          mempty
            <> "listing transactions from "
  ...
  it "TRANS_LIST_03 - Can order results" $ \ctx -> runResourceT $ do
    let
      a1 = Quantity $ sum $ replicate 10 (minUTxOValue (_mainEra ctx))
    let
      a2 = Quantity $ sum $ replicate 10 (2 * (minUTxOValue (_mainEra ctx)))
    w <-
    ...
    forM_ orderings $ \(order, expects) -> do
      let
        args = T.unpack <$> w ^. walletId : order
      (Exit code, Stdout out, Stderr err) <-
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      t <- unsafeGetTransactionTime =<< listAllTransactions @n ctx w
      let
        tl = utcIso8601ToText $ utcTimeSucc t
      Stdout o1 <-
        listTransactionsViaCLI
          ctx
          TxMetadataDetailedSchema
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      t <- unsafeGetTransactionTime =<< listAllTransactions @n ctx w
      let
        te = utcIso8601ToText $ utcTimePred t
      Stdout o1 <-
        listTransactionsViaCLI
          ctx
          TxMetadataDetailedSchema
  ...
  it "TRANS_GET_01 - Can get Incoming and Outgoing transaction" $ \ctx -> runResourceT $ do
    ...
    eventually "Transactions are available and in ledger" $ do
      ...
      let
        wDestId = T.unpack (wDest ^. walletId)
