...
spec = describe "BYRON_CLI_WALLETS" $ do
  ...
  describe
    ...
    $ do
      let
        scenarioSuccess style mnemonic ctx = runResourceT @IO $ do
          let
      ...
      let
        scenarioFailure style mnemonic ctx = runResourceT @IO $ do
          let
            args =
  ...
  it "CLI_BYRON_UPDATE_PASS_01 - change passphrase" $ \ctx ->
    forM_ [emptyRandomWallet, emptyIcarusWallet]
      $ \emptyByronWallet -> runResourceT @IO $ do
        ...
        let
          oldPass = T.unpack fixturePassphrase
        let
          newPass = "cardano-wallet-new-pass"
        (c, o, e) <-
  ...
  describe "CLI_BYRON_UPDATE_PASS_03 - Pass length incorrect" $ do
    ...
    let
      maxLength = passphraseMaxLength (Proxy @"user")
    let
      passTooShort = replicate (minLength - 1) 'o'
    let
      errMsgTooShort = "passphrase is too short: expected at least 10 characters"
    let
      passTooLong = replicate (maxLength + 1) 'o'
    let
      errMsgTooLong = "passphrase is too long: expected at most 255 characters"
    let
      passOK = T.unpack fixturePassphrase
