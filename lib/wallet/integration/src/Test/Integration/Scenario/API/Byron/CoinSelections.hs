...
spec = describe "BYRON_COIN_SELECTION" $ do
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        payments = pure (AddressAmount addr amt mempty)
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        amt = Quantity . minUTxOValue $ _mainEra ctx
      let
        payment = AddressAmount targetAddress amt mempty
      let
        output = ApiCoinSelectionOutput targetAddress amt mempty
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        payments = pure (AddressAmount addr minUTxOValue' mempty)
      _ <- request @ApiByronWallet ctx (Link.deleteWallet @'Byron icW) Default Empty
      selectCoins @_ @'Byron ctx icW payments
        >>= flip
          verify
