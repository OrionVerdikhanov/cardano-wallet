...
spec = describe "NEW_SHELLEY_TRANSACTIONS" $ do
  ...
  it "TRANS_NEW_CREATE_02a - Only metadata" $ \ctx -> runResourceT $ do
    wa <- fixtureWallet ctx
    let
      metadata = Json [json|{ "metadata": { "1": { "string": "hello" } } }|]
    ...
    let
      tx = cardanoTxIdeallyNoLaterThan era $ getApiT (signedTx ^. #serialisedTxSealed)
    ...
    let
      decodePayload = Json (toJSON signedTx)
    let
      expMetadata =
  ...
  it "TRANS_NEW_CREATE_02b - Only metadata, untyped"
    $ \ctx -> runResourceT $ do
      ...
      let
        decodePayload = Json (toJSON signedTx)
      let
        expMetadata = ApiT (TxMetadata (Map.fromList [(1, TxMetaText "hello")]))
      rDecodedTx <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shelley wa)
  ...
  it "TRANS_NEW_CREATE_03a - Withdrawal from self, 0 rewards" $ \ctx -> runResourceT $ do
    wa <- fixtureWallet ctx
    let
      initialBalance = wa ^. #balance . #available . #getQuantity
    let
      withdrawal = Json [json|{ "withdrawal": "self" }|]
  ...
  it "TRANS_NEW_CREATE_03a - Withdrawal from self" $ \ctx -> runResourceT $ do
    ...
    let
      withdrawal = Json [json|{ "withdrawal": "self" }|]
    let
      withdrawalAmt = 1_000_000_000_000
    let
      rewardInitialBalance = 100_000_000_000
    ...
    let
      decodePayload = Json (toJSON signedTx)
    let
      withdrawalWith ownership wdrls = case wdrls of
        [wdrl] ->
  ...
  it "TRANS_NEW_CREATE_04a - Single Output Transaction with decode transaction" $ \ctx -> runResourceT $ do
    ...
    let
      txCborHex = getFromResponse #transaction rTxHex
    let
      decodePayloadHex = Json (toJSON txCborHex)
    rDecodedTxSourceHex <-
      request @(ApiDecodedTransaction n)
        ctx
        (Link.decodeTransaction @'Shelley wa)
  ...
  it "TRANS_NEW_CREATE_04ab - Constructed inputs = Decoded inputs" $ \ctx -> runResourceT $ do
    ...
    let
      expectedInputs = getFromResponse (#coinSelection . #inputs) rTx
    let
      txCbor = getFromResponse #transaction rTx
    let
      decodePayload = Json (toJSON txCbor)
    ...
    rDecodedTxSource <-
      request @(ApiDecodedTransaction n)
        ...
        decodePayload
    let
      decodedInputs = getFromResponse #inputs rDecodedTxSource
  ...
  it "TRANS_NEW_CREATE_04a - Single Output Transaction with submitWithWid" $ \ctx -> runResourceT $ do
    ...
    let
      queryTx = Link.getTransaction @'Shelley wa (ApiTxId txid)
  ...
  it "TRANS_NEW_ASSETS_CREATE_01a - Multi-asset tx with Ada" $ \ctx -> runResourceT $ do
    ...
    let
      assetsSrc = wa ^. #assets . #total . #getApiT
    ...
    let
      amt = 2 * minUTxOValue (_mainEra ctx)
  ...
  it "TRANS_NEW_ASSETS_CREATE_01c - Multi-asset tx without Ada" $ \ctx -> runResourceT $ do
    ...
    outTxAmt <- eventually "Transactions is in ledger" $ do
      let
        linkSrc = Link.getTransaction @'Shelley wa txId
  ...
  it "TRANS_NEW_ASSETS_CREATE_01d - Multi-asset tx with not enough assets" $ \ctx -> runResourceT $ do
    ...
    assetsSrc `shouldNotBe` mempty
    let
      val = (minUTxOValue' * minUTxOValue') <$ pickAnAsset assetsSrc
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        apiTx'@(ApiSerialisedTransaction apiTx _) =
          getFromResponse #transaction rTx
      let
        decodePayload1 = Json (toJSON apiTx')
      rDecodedTx1 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shelley wa)
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        initialAmt = minUTxOValue (_mainEra ctx)
      wa <- fixtureWalletWith @n ctx [initialAmt]
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        cborHexMint = fromTextEnvelope cborHexWithMinting
      let
        decodeMintPayload =
          Json
            [json|{
      ...
      let
        tokenName' = UnsafeTokenName "HappyCoin"
      let
        policyWithHash = Link.getPolicyKey @'Shelley wa (Just True)
      (_, policyKeyHashPayload) <-
        unsafeRequest @ApiPolicyKey ctx policyWithHash Empty
      let
        (Just externalPolicyKeyHash) =
          keyHashFromBytes
            ( Policy
            , "i0<\227Sm\242`\239\221\188\148\156\203\148\230\153\&3\STX\177\vw\141\139M\152\191\181"
            )
      let
        scriptUsed = RequireAllOf [RequireSignatureOf externalPolicyKeyHash]
      ...
      let
        cborHexBurn = fromTextEnvelope cborHexWithBurning
      let
        decodeBurnPayload =
          Json
            [json|{
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        cborHexSettingUpReferenceScript =
      ...
      let
        refInp = ReferenceInput $ TxIn (Hash txId) 0
      let
        plutusScript =
      ...
      let
        tokenName' = UnsafeTokenName "ReferencePlutusScriptAsset"
      let
        tokenPolicyId' = UnsafeTokenPolicyId $ Hash plutusScriptHash
      let
        apiTokenAmountFingerprint =
          ApiTokenAmountFingerprint
            { assetName = ApiT tokenName'
      ...
      let
        refScript = AnyScriptReference (ScriptHash plutusScriptHash) [refInp]
      let
        apiTokens =
          ApiTokens
            { policyId = ApiT tokenPolicyId'
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        tokenName' = UnsafeTokenName "ReferenceSimpleScriptAsset"
      let
        (ScriptHash nativeScriptHash) = toScriptHash nativeScript
      let
        tokenPolicyId' = UnsafeTokenPolicyId $ Hash nativeScriptHash
      let
        apiTokenAmountFingerprint =
          ApiTokenAmountFingerprint
            { assetName = ApiT tokenName'
      ...
      let
        refScript = AnyScriptReference (ScriptHash nativeScriptHash) [refInp]
      let
        apiTokens =
          ApiTokens
            { policyId = ApiT tokenPolicyId'
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        cborHexMIR = fromTextEnvelope cborHex

      let
        containMIR certs = OtherCertificate (ApiT MIRCertificate) `elem` certs
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        cborHexPool = fromTextEnvelope cborHex

      let
        (Right percentage) = mkPercentage (1 % 10)
      let
        poolId' =
          PoolId
  ...
  it "TRANS_NEW_BALANCE_02a - Cannot balance on empty wallet"
    $ \ctx -> runResourceT $ do
      wa <- emptyWallet ctx
      let
        balancePayload = Json PlutusScenario.pingPong_1
      rTx <-
        request @ApiSerialisedTransaction
          ctx
          (Link.balanceTransaction @'Shelley wa)
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        toBalance = Json PlutusScenario.pingPong_1
      rTx <-
        request @ApiSerialisedTransaction
          ctx
          (Link.balanceTransaction @'Shelley wa)
      ...
      let
        apiTx = getFromResponse #serialisedTxSealed rTx
      signedTx <- signTx ctx wa apiTx [expectResponseCode HTTP.status202]
      submittedTx <- submitTxWithWid ctx wa signedTx
      verify
        submittedTx
      ...
      partialTx' <-
        PlutusScenario.pingPong_2
          $ Aeson.object
            ["transactionId" .= txId]
      let
        toBalance' = Json (toJSON partialTx')
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        balancePayload = Json PlutusScenario.pingPong_1
      let
        hasExpectedFee = [expectField (#fee . #getQuantity) (`shouldBe` expectedFee)]
      ...
      let
        apiTx = getFromResponse #serialisedTxSealed rTx
      let
        serTx = getFromResponse Prelude.id rTx
      let
        decodePayload = Json (toJSON serTx)
  ...
  it "TRANS_NEW_SIGN_01 - Sign single-output transaction" $ \ctx -> runResourceT $ do
    ...
    let
      constructEndpoint = Link.createUnsignedTransaction @'Shelley w
    sealedTx <-
  ...
  it "TRANS_NEW_SIGN_03 - Sign withdrawals" $ \ctx -> runResourceT $ do
    ...
    -- Construct tx
    let
      payload = Json [json|{"withdrawal": "self"}|]
    let
      constructEndpoint = Link.createUnsignedTransaction @'Shelley w
    (_, apiTx) <-
      unsafeRequest @(ApiConstructTransaction n) ctx constructEndpoint payload
    length (withdrawals $ coinSelection apiTx) `shouldBe` 1

    -- Sign tx
    let
      sealedTx = apiTx ^. #transaction
      toSign =
        Json
          [json|
                { "transaction": #{serialisedTxSealed sealedTx}
  ...
  it "TRANS_NEW_SIGN_04 - Sign extra required signatures" $ \ctx -> runResourceT $ do
    ...
    let
      apiTx' = ApiT (apiTx `addRequiredSigners` [addrVk])
  ...
  describe "TRANS_NEW_SUBMIT_01 - Submitting on foreign wallet is forbidden" $ do
    ...
    forM_ scenarios $ \(title, foreignWallet) -> it title $ \ctx -> runResourceT $ do
      ...
      let
        constructEndpoint = Link.createUnsignedTransaction @'Shelley wa
      sealedTx <-
        getFromResponse #transaction
          <$> request @(ApiConstructTransaction n) ctx constructEndpoint Default payload
      ...
      let
        signEndpoint = Link.signTransaction @'Shelley wa
      signedTx <-
        getFromResponse Prelude.id
          <$> request @ApiSerialisedTransaction ctx signEndpoint Default toSign
  ...
  describe "TRANS_NEW_SUBMIT_02 - Submitting on foreign Byron wallet is forbidden" $ do
    ...
    forM_ scenarios $ \(title, foreignByronWallet) -> it title $ \ctx -> runResourceT $ do
      ...
      -- Submit tx (from wb)
      let
        submitEndpoint = ("POST", "v2/wallets/" <> wid <> "/transactions-submit")
      let
        submitPayload = Json $ Aeson.toJSON signedTx
  ...
  it "TRANS_NEW_SUBMIT_03 - Can submit transaction encoded in base16" $ \ctx -> runResourceT $ do
    ...
    signedTx <-
      getFromResponse #serialisedTxSealed
        <$> request @ApiSerialisedTransaction ctx signEndpoint Default toSign
    let
      signedTxHex = PlutusScenario.toHex $ serialisedTx $ getApiT signedTx

    -- Submit tx
    let
      submitEndpoint = Link.submitTransaction @'Shelley wa
    let
      toSend =
        Json
          [json|
  ...
  describe "Plutus scenarios" $ do
    let
      scenarios =
        ...
        ,
          ...
          , \ctx _w -> do
              let
                (script, _scriptHash) = PlutusScenario.alwaysTrueValidator
              liftIO
          ...
          , \ctx w -> do
              ...
              let
                getFreshUTxO = do
                  ...
                  let
                    singleton = pure -- specialized to NonEmpty
                  (_, result) <-
              ...
              let
                mint = PlutusScenario.currencyTx txOutRef
              pure (mint, [])
          , [expectResponseCode HTTP.status202]
          )
        ]
    ...
    forM_ scenarios $ \(title, setupContract, decodeExp) -> it title $ \ctx -> runResourceT $ do
      ...
      let
        txid = getFromResponse Prelude.id submittedTx

      let
        runStep previous step = do
          waitForTxImmutability ctx
  ...
  it "TRANS_NEW_JOIN_01a - Can join stakepool, rejoin another and quit" $ \ctx -> runResourceT $ do
    let
      initialAmt = 10 * minUTxOValue (_mainEra ctx)
    ...
    let
      (ApiSerialisedTransaction apiTx1 _) = getFromResponse #transaction rTx1
    ...
    let
      decodePayload1 = Json (toJSON signedTx1)
    rDecodedTx1 <-
      request @(ApiDecodedTransaction n)
        ctx
    ...
    let
      txId1 = getFromResponse #id submittedTx1
    let
      link = Link.getTransaction @'Shelley src (ApiTxId txId1)
    ...
    let
      (ApiSerialisedTransaction apiTx2 _) = getFromResponse #transaction rTx2
    ...
    let
      decodePayload2 = Json (toJSON signedTx2)
    rDecodedTx2 <-
      request @(ApiDecodedTransaction n)
        ctx
        (Link.decodeTransaction @'Shelley src)
    ...
    let
      queryTx2 = Link.getTransaction @'Shelley src (ApiTxId txid2)
    ...
    eventually "Certificates are inserted" $ do
      let
        ep = Link.listTransactions @'Shelley src
    ...
    let
      (ApiSerialisedTransaction apiTx4 _) = getFromResponse #transaction rTx4
    ...
    let
      txid3 = getFromResponse (#id) submittedTx4
    let
      queryTx3 = Link.getTransaction @'Shelley src (ApiTxId txid3)
  ...
  it "TRANS_NEW_JOIN_01b - Invalid pool id" $ \ctx -> runResourceT $ do
    wa <- fixtureWallet ctx
    let
      invalidPoolId = T.replicate 32 "1"
    let
      delegation =
        Json
          [json|{
  ...
  it "TRANS_NEW_JOIN_01b - Absent pool id" $ \ctx -> runResourceT $ do
    wa <- fixtureWallet ctx
    let
      absentPoolIdBech32 = "pool1mgjlw24rg8sp4vrzctqxtf2nn29rjhtkq2kdzvf4tcjd5pl547k"
  ...
  it "TRANS_NEW_JOIN_01c - Multidelegation not supported" $ \ctx -> runResourceT $ do
    wa <- fixtureWallet ctx
    let
      pool' = "pool1mgjlw24rg8sp4vrzctqxtf2nn29rjhtkq2kdzvf4tcjd5pl547k" :: Text
    let
      delegations =
        Json
          [json|{
  ...
  it "TRANS_NEW_QUIT_02a - Cannot quit with rewards without explicit withdrawal"
    $ \ctx -> runResourceT $ do
      ...
      (ApiSerialisedTransaction unsignedTx1 _) <-
        view #transaction . snd
      ...
      submitTxWithWid ctx w signedTx1
        >>= flip
          verify
          [ expectSuccess
          , expectResponseCode HTTP.status202
    ...
    $ \ctx -> runResourceT $ do
      ...
      rUnsignedTx2 <-
        request @(ApiConstructTransaction n)
          ...
          payload2
      let
        (ApiSerialisedTransaction unsignedTx2 _) =
      ...
      submitTxWithWid ctx w signedTx2
        >>= flip
          verify
          [ expectSuccess
          , expectResponseCode HTTP.status202
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      eventually "Source wallet balance is as expected" $ do
        let
          balance = fromIntegral oneMillionAda - amt * 2 - expectedFee - deposit
        rWa <-
          request @ApiWallet
            ctx
            (Link.getWallet @'Shelley wa)
  ...
  it "TRANS_NEW_CREATE_10l - Minting when assetName too long"
    $ \ctx -> runResourceT $ do
      ...
      let
        destination = (addrs !! 1) ^. #id
      let
        assetNameTooLong = replicate 66 '1'
      let
        payload =
          Json
            [json|{
  ...
  it "TRANS_NEW_CREATE_10d - Minting assets without timelock"
    $ \ctx -> runResourceT $ do
      ...
      let
        (Right tokenName') = mkTokenName "ab12"
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        minutxo = (minUTxOValue (_mainEra ctx) :: Natural)
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        postPolicyId = Link.postPolicyId @'Shelley wa
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        scriptUsed = RequireAllOf [RequireSignatureOf policyKeyHash]
      let
        tokenPolicyId' =
  ...
  it "TRANS_NEW_LIST_05 - filter address output side" $ \ctx -> runResourceT $ do
    ...
    eventually "There are exactly 6 transactions for wDest" $ do
      let
        query = listTransactionsFilteredByAddress wDest Nothing
    ...
    let
      addr0 = (head addrs) ^. #id
    let
      query0 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr0))
    ...
    let
      addr1 = (addrs !! 1) ^. #id
    let
      query1 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr1))
    ...
    let
      addr2 = (addrs !! 2) ^. #id
    let
      query2 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr2))
  ...
  where
    ...
    listExternalAddresses ctx w = do
      let
        link = Link.listAddresses @'Shelley w
    ...
    sendAmtToAddr ctx src dest amt addrIx = do
      ...
      let
        txid = getFromResponse #id submittedTx
      let
        queryTx w = Link.getTransaction @'Shelley w (ApiTxId txid)
    ...
    mintAssetsCheckWithoutBalanceCheck
      ...
      scriptUsedF = do
        ...
        let
          postPolicyKey = Link.postPolicyKey @'Shelley wa (Just True)
        let
          passwdPayload =
            Json
              [json| {
        ...
        let
          scriptUsed = scriptUsedF policyKeyHash
        let
          tokenPolicyId' =
            UnsafeTokenPolicyId . Hash
              $ unScriptHash
