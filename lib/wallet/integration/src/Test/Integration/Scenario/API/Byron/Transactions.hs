...
spec = describe "BYRON_TRANSACTIONS" $ do
  describe "BYRON_TRANS_ASSETS_CREATE_01 - Multi-asset transaction with ADA"
    ...
    $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
      ...
      let
        assetsSrc = wSrc ^. #assets . #total . #getApiT
      assetsSrc `shouldNotBe` mempty
      let
        minUTxOValue' = minUTxOValue (_mainEra ctx)
      let
        val = minUTxOValue' <$ pickAnAsset assetsSrc
  ...
  describe "BYRON_TRANS_ASSETS_LIST_02 - Asset list present when not used"
    $ forM_
      [ (fixtureRandomWallet, "Byron fixture wallet")
  ...
  describe "BYRON_TRANS_ASSETS_GET_01 - Asset list present"
    ...
    $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
      ...
      let
        tokenFingerprint = mkTokenFingerprint polId assName
      let
        ep = Link.getByronAsset wal polId assName
  ...
  describe "BYRON_TRANS_ASSETS_GET_02a - Asset not present when isn't associated"
    ...
    $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
      ...
      let
        polId = TokenPolicy.UnsafeTokenPolicyId $ Hash $ BS.replicate 28 0
      let
        ep = Link.getByronAsset wal polId TokenPolicy.nullTokenName
      r <- request @(ApiAsset) ctx ep Default Empty
      expectResponseCode HTTP.status404 r
      expectErrorMessage errMsg404NoAsset r
  ...
  describe "BYRON_TRANS_CREATE_01 - Single Output Transaction Byron -> Shelley"
    ...
    $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
      ...
      rFeeEst <-
        request @ApiFee
          ctx
          (Link.getTransactionFeeOld @'Byron wByron)
          Default
          payload
      verify
        rFeeEst
      ...
      let
        feeEstMin = getFromResponse (#estimatedMin . #getQuantity) rFeeEst
      let
        feeEstMax = getFromResponse (#estimatedMax . #getQuantity) rFeeEst
      ...
      eventually "wa and wb balances are as expected as well as fees on txs" $ do
        ...
        rTxSrc <-
          request @(ApiTransaction n)
            ctx
            (Link.getTransaction @'Byron wByron (ApiTxId txId))
            Default
            Empty
        verify
          rTxSrc
        ...
        rTxDest <-
          request @(ApiTransaction n)
            ctx
            (Link.getTransaction @'Shelley wShelley (ApiTxId txId))
            Default
            Empty
        verify
          rTxDest
  ...
  describe "BYRON_TRANS_CREATE_01a - Single Output Transaction Byron -> Byron"
    ...
    $ \(emptyByronWallet, name) -> it name $ \ctx -> runResourceT $ do
      ...
      let
        walType = getFromResponse #discovery ra

      destination <- case walType of
        DiscoveryRandom -> do
          let
            payloadAddr = Json [json| { "passphrase": #{fixturePassphrase} }|]
          rA <-
        ...
        DiscoverySequential -> do
          let
            link = Link.listAddresses @'Byron wDestByron
          rA2 <- request @[ApiAddressWithPath n] ctx link Default Empty
          let
            addrs = getFromResponse Prelude.id rA2
          pure $ (addrs !! 1) ^. #id

      let
        amt = 2 * minUTxOValue (_mainEra ctx) :: Natural
      let
        payload =
          Json
            [json|{
      ...
      let
        (Quantity feeEstMin) = getFromResponse #estimatedMin rFeeEst
      let
        (Quantity feeEstMax) = getFromResponse #estimatedMax rFeeEst
      ...
      rTx <-
        request @(ApiTransaction n)
          ctx
          (Link.createTransactionOld @'Byron wByron)
          Default
          payloadTx
      verify
        rTx
      ...
      eventually "wa and wb balances and fees are as expected" $ do
        ...
        rTxDest <-
          request @(ApiTransaction n)
            ctx
            (Link.getTransaction @'Byron wDestByron (ApiTxId txId))
            Default
            Empty
        verify
          rTxDest
        ...
        rDest <-
          request @ApiByronWallet
            ctx
            (Link.getWallet @'Byron wDestByron)
            Default
            Empty
        verify
          rDest
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        endpoint = "v2/wallets/" <> wid <> "/transactions"
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        endpoint = "v2/wallets/" <> wid <> "/transactions/" <> txid
      r <- request @ApiTxId ctx ("DELETE", endpoint) Default Empty
      expectResponseCode HTTP.status404 r
      expectErrorMessage (errMsg404NoWallet wid) r

  it
    "BYRON_TRANS_ESTIMATE -\
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        endpoint = "v2/wallets/" <> wid <> "/payment-fees"
  ...
  it
    "BYRON_TX_LIST_02 -\
    \ Byron endpoint does not list Shelley wallet transactions"
    $ \ctx -> runResourceT $ do
      w <- emptyWallet ctx
      let
        wid = w ^. walletId
      let
        ep = ("GET", "v2/byron-wallets/" <> wid <> "/transactions")
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        wid = w ^. walletId
      let
        ep = ("GET", "v2/wallets/" <> wid <> "/transactions")
      r <- request @([ApiTransaction n]) ctx ep Default Empty
      verify
        r
        [ expectResponseCode HTTP.status404
  ...
  it "BYRON_TX_LIST_01 - 0 txs on empty Byron wallet"
    $ \ctx -> runResourceT @IO $ forM_ [emptyRandomWallet, emptyIcarusWallet] $ \emptyByronWallet -> do
      ...
      let
        link = Link.listTransactions @'Byron w
  ...
  describe "BYRON_TX_LIST_01 - Faulty start, end, order values" $ do
    ...
    forM_ queries $ \tc -> it (T.unpack $ query tc) $ \ctx -> runResourceT @IO $ do
      w <- emptyRandomWallet ctx
      let
        link = withQuery (query tc) $ Link.listTransactions @'Byron w
      r <- request @([ApiTransaction n]) ctx link Default Empty
  ...
  describe "BYRON_TX_LIST_ADDRESS - Transactions can be filtered by address"
    $ forM_
      [ (fixtureRandomWallet, emptyRandomWallet, "Byron wallet")
    ...
    $ \(srcFixture, srcEmpty, name) -> it name $ \ctx -> runResourceT $ do
      ...
      wDest <- srcEmpty ctx
      let
        link w = listTransactionsFilteredByAddress w Nothing
      ...
      let
        linkList3 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr2))
      ...
      case walType of
        DiscoveryRandom -> do
          ...
          rA <-
            request @(ApiAddressWithPath n)
              ctx
              (Link.postRandomAddress wal)
              Default
              payloadAddr
          pure $ getFromResponse #id rA
        DiscoverySequential -> do
          let
            link = Link.listAddresses @'Byron wal
          rA2 <- request @[ApiAddressWithPath n] ctx link Default Empty
          let
            addrs = getFromResponse Prelude.id rA2
    ...
    sendAmtToNewAddr ctx src dest amt addrIx = do
      ...
      eventually "Tx is in ledger finally for dest wallet" $ do
        rTxDest <-
          request @(ApiTransaction n)
            ctx
            (Link.getTransaction @'Byron dest (ApiTxId txId))
            Default
            Empty
        verify
          rTxDest
      ...
      eventually "Tx is in ledger finally for src wallet" $ do
        rTxDest <-
          request @(ApiTransaction n)
            ctx
            (Link.getTransaction @'Byron src (ApiTxId txId))
            Default
            Empty
        verify
          rTxDest
