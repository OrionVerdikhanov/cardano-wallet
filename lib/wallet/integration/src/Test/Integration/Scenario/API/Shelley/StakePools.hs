...
spec = describe "SHELLEY_STAKE_POOLS" $ do
  ...
  it "STAKE_POOLS_JOIN_01 - Cannot join non-existent wallet" $ \ctx -> runResourceT $ do
    ...
    _ <-
      request @ApiWallet
        ...
        Empty
    let
      poolIdAbsent = PoolId $ BS.pack $ replicate 32 1
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        txId = getFromResponse #id rJoin
      let
        link = Link.getTransaction @'Shelley src (ApiTxId txId)
      ...
      let
        txAmount = getFromResponse #amount rTx
      ...
      let
        txid = getFromResponse Prelude.id rq
      let
        quitFeeAmt = getFromResponse #amount rq

      eventually "Certificates are inserted after quitting a pool" $ do
        let
          epg = Link.getTransaction @'Shelley src txid
        ...
        let
          epl = Link.listTransactions @'Shelley src
  ...
  describe "STAKE_POOLS_JOIN_UNSIGNED_01" $ do
    it "Can join a pool that's not retiring" $ \ctx -> runResourceT $ do
      ...
      let
        isValidCerts (Just (RegisterRewardAccount {} :| [JoinPool {}])) =
          True
        isValidCerts _ =
          False
  ...
  describe "STAKE_POOLS_QUIT_UNSIGNED_01"
    $ it "Join/quit when already joined a pool"
    $ \ctx -> runResourceT $ do
      w <- fixtureWallet ctx
      ...
      liftIO
        $ joinStakePoolUnsigned @n @'Shelley ctx w (ApiT pool1) >>= \o ->
          verify
            o
            [ expectResponseCode HTTP.status403
      ...
      let
        isValidCertsJoin (Just (JoinPool {} :| [])) = True
        isValidCertsJoin _ = False
      liftIO
        $ joinStakePoolUnsigned @n @'Shelley ctx w (ApiT pool2) >>= \o ->
          verify
            o
            [ expectResponseCode HTTP.status200
            , expectField
      ...
      -- Can quit pool
      let
        isValidCertsQuit (Just (QuitPool {} :| [])) = True
        isValidCertsQuit _ = False
      liftIO
        $ quitStakePoolUnsigned @n @'Shelley ctx w >>= \o ->
          verify
            o
            [ expectResponseCode HTTP.status200
            , expectField
  ...
  describe "STAKE_POOLS_QUIT_01x - Fee boundary values" $ do
    ...
    it
      ...
      $ \ctx -> runResourceT $ do
        let
          initBalance = [costOfJoining ctx + depositAmt ctx]
        w <- fixtureWalletWith @n ctx initBalance
  ...
  describe "STAKE_POOLS_LIST_01 - List stake pools" $ do
    ...
    it "eventually has correct margin, cost and pledge" $ \ctx -> runResourceT $ do
      eventually "pool worker finds the certificate" $ do
        ...
        let
          pools' = either (error . show) Prelude.id $ snd r
    ...
    it "non-myopic-rewards are based on stake" $ \ctx -> runResourceT $ do
      eventually "rewards are smaller for smaller stakes" $ do
        let
          stakeSmall = Just (Coin 1_000)
        let
          stakeBig = Just (Coin 10_000_000_000_000_000)
        ...
        let
          rewardsStakeSmall = sum (rewards <$> poolsStakeSmall)
        let
          rewardsStakeBig = sum (rewards <$> poolsStakeBig)
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        certificates = poolGarbageCollectionCertificates =<< events
      ...
      let
        epochs = poolGarbageCollectionEpochNo <$> events
  ...
  it "STAKE_POOLS_SMASH_HEALTH_03 - Can check SMASH health via url"
    $ \ctx -> runResourceT $ do
      let
        withUrl f (method, link) = (method, link <> "?url=" <> f)
      let
        link = withUrl (_smashUrl ctx) Link.getCurrentSMASHHealth
  ...
  describe "STAKE_POOLS_SMASH_HEALTH_04 - SMASH url needs to be valid" $ do
    ...
    forM_ m $ \(url, message) -> it url $ \ctx -> runResourceT $ do
      let
        withUrl f (method, link) = (method, link <> "?url=" <> T.pack f)
      let
        link = withUrl url Link.getCurrentSMASHHealth
  ...
  it "STAKE_KEY_LIST_01 - Can list stake keys" $ \ctx -> runResourceT $ do
    w <- fixtureWallet ctx
    let
      balance = Quantity 1_000_000_000_000
  ...
  it "STAKE_KEY_LIST_02 - Can list foreign stake key from UTxO" $ \ctx -> runResourceT $ do
    ...
    let
      ourAddr' = replaceStakeKey ourAddr foreignAddr
    let
      payload =
        [json|
                { "payments":
