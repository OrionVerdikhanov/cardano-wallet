...
spec = describe "SHELLEY_MIGRATIONS" $ do
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        ep = Link.createMigrationPlan @'Shelley sourceWallet
      response <-
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      let
        perEntryAdaQuantity = Coin $ case _mainEra ctx of
          e
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      --
      let
        perEntryAdaQuantity = Coin 1_562_500
      let
        perEntryAssetCount = 1
      let
        batchSize = 20
      sourceAddresses <-
      ...
      let
        expectedBalanceAda = 387_500_000
      let
      ...
      let
        expectedSourceDistributionAfterMinting =
          [ (10_000_000, 120)
          , (100_000_000, 2)
          ]
      request @ApiUtxoStatistics
      ...
      request @(ApiWalletMigrationPlan n)
        ...
        >>= flip
          ...
          , expectField
              ...
              (`shouldBe` expectedBalanceAda)
          , expectField
              (#balanceSelected . #assets . #getApiT)
              ((`shouldBe` expectedAssetCount) . TokenMap.size)
          , expectField
              (#balanceLeftover . #ada . #getQuantity)
              (`shouldBe` 0)
          , expectField
              (#balanceLeftover . #assets . #getApiT)
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      --
      let
        perEntryAdaQuantity = Coin 1_462_500
      let
        perEntryAssetCount = 1
      let
        batchSize = 20
      ...
      -- distribution have both changed accordingly:
      let
        expectedBalanceAda = 275_500_000
      let
        expectedAssetCount = 20
      request @ApiWallet
        ctx
      ...
      let
        targetAddressIds =
          targetAddresses
            <&> (\(ApiTypes.ApiAddressWithPath addrId _ _) -> addrId)
      let
        ep = Link.migrateWallet @'Shelley sourceWallet
      response <-
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      let
        endpointPlan = (Link.createMigrationPlan @'Shelley sourceWallet)
      responsePlan <-
        request @(ApiWalletMigrationPlan n)
          ctx
          endpointPlan
      ...
      -- Perform a migration:
      let
        endpointMigrate = Link.migrateWallet @'Shelley sourceWallet
      responseMigrate <-
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      -- Check that funds become available in the target wallet:
      let
        expectedBalance = sourceBalance - feeExpected
      request @ApiWallet
        ctx
        (Link.getWallet @'Shelley targetWallet)
        Default
      ...
      -- Analyse the target wallet's UTxO distribution:
      let
        expectedTargetDistribution = [(100_000_000, 1)]
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      -- Check that the source wallet has the expected balance:
      let
        expectedAdaBalanceAvailable = 100_000_000_000
      let
        expectedAdaBalanceReward = 1_000_000_000_000
      let
        expectedAdaBalanceTotal = 1_100_000_000_000
      request @ApiWallet
        ctx
        (Link.getWallet @'Shelley sourceWallet)
        Default
      ...
      -- Check that funds become available in the target wallet:
      let
        expectedTargetBalance = expectedAdaBalanceTotal - expectedFee
      request @ApiWallet
        ctx
        (Link.getWallet @'Shelley targetWallet)
        Default
  ...
  Hspec.it
    ...
    $ \ctx -> runResourceT @IO $ do
      ...
      -- Check that funds become available in the target wallet:
      let
        expectedTargetBalance = expectedAdaBalance - expectedFee
      request @ApiWallet
        ctx
        (Link.getWallet @'Shelley targetWallet)
        Default
  ...
  where
    ...
    testAddressCycling targetAddressCount = do
      ...
      it title $ \ctx -> runResourceT $ do
        ...
        let
          expectedTargetBalance = sourceBalance - expectedFee
        response2 <-
          request @ApiWallet
            ctx
            (Link.getWallet @'Shelley targetWallet)
